...
timer0:
	...
	;------------------
	LDI R18, 10					; set loop counter / Load Immediate (Loads an 8-bit constant directly to register 16 to 31)
L1: RCALL timer0_delay			; apply delay via timer0 / – Relative Call to Subroutine (Relative call to an address within PC-2K+1 and PC+2K {words}. The return address {the instruction after the RCALL} is stored onto the Stack)
	...
	LDI R18, 10					; re-set loop counter / Load Immediate (Loads an 8-bit constant directly to register 16 to 31)
	RJMP L1						; go back & repeat toggle / Relative Jump (Relative jump to an address within PC-2K+1 and PC+2K {words})
	;==================
timer0_delay:					; 16.4 ms delay via timer0
	;------------------
	...
	LDI R20, 0b00000101			;R20 = 0b00000101 / Load Immediate (Loads an 8-bit constant directly to register 16 to 31)
	OUT TCCR0B, R20				; timer0: normal mode, prescaler 1024 / Timer-Counter Control Register B
	;------------------
L2: IN R20, TIFR0				; get TIFR0 byte & check / Load an I/O Location to Register (Loads data from the I/O Space {Ports, Timers, Configuration Registers, etc.} into register Rd in the Register File)
	SBRS R20, TOV0				; if TOV0=1, skip next instruction / Skip if Bit in Register is Set (This instruction tests a single bit in a register and skips the next instruction if the bit is set)
	...
	;------------------
	LDI R20, (1<<TOV0)			; Load Immediate (Loads an 8-bit constant directly to register 16 to 31)
	OUT TIFR0, R20				; clear TOV0 flag / Store Register to I/O Location
	RET 
